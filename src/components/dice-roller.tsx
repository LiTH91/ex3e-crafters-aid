...
const getDieStyle = (roll: number, activeCharms: string[]): string => {
  ...
  if (isSpecialSuccess) return "bg-yellow-400 text-black border-yellow-600";
  if (roll >= 7) return "bg-green-500 text-white border-green-700";
    ...
    <div className="flex items-center justify-center gap-4 flex-wrap p-4 bg-secondary/30 rounded-lg">
        {waves.map((wave, waveIndex) => (
           <div key={`wave-${waveIndex}`} className="flex items-center gap-4">
                ...
                {waveIndex < waves.length - 1 && <ArrowRight className="w-6 h-6 text-muted-foreground hidden md:block" />}
           </div>
        ))}
    </div>
);

const DiceRoller = ({ character, activeProjects }: DiceRollerProps) => {
  const [diceRoll, setDiceRoll] = useState<DiceRoll | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  const [targetNumber, setTargetNumber] = useState(7);
  const [willpowerSpent, setWillpowerSpent] = useState(0);
  const [activeCharms, setActiveCharms] = useState<string[]>([]); // This needs to be connected to CharmSelection
  const [aiOutcome, setAiOutcome] = useState<CraftingOutcome | null>(null); // This is for a different feature
  ...
  const handleRollClick = useCallback(async () => {
    ...
    toast({
      title: "Roll Complete!",
      description: "The dice have settled.",
    });
  }, [character, activeCharms, targetNumber, willpowerSpent, toast]);

  const dicePool = character[character.selectedAttribute] + character.craft;

  const formatProjectTypeName = (type: ProjectType) => type.split("-").map(s => s.charAt(0).toUpperCase() + s.slice(1)).join(" ");

  const activeNarrativeCharms = useMemo(() => {
    if (!diceRoll || !diceRoll.activeCharmIds) return [];
    return allCharms.filter(
      (charm) => diceRoll.activeCharmIds.includes(charm.id) && charm.category === 'narrative'
    );
  }, [diceRoll]);
    ...
    <Card className="bg-card/80 backdrop-blur-sm border-2 border-primary/20 shadow-lg">
      ...
      <CardContent className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
          <div>
            <Label htmlFor="project-type" className="font-bold">Project Type</Label>
            <Select value={projectType} onValueChange={(v) => setProjectType(v as ProjectType)}>
              <SelectTrigger id="project-type"><SelectValue placeholder="Select type" /></SelectTrigger>
              <SelectContent>{PROJECT_TYPES.map((type) => (<SelectItem key={type} value={type}>{formatProjectTypeName(type)}</SelectItem>))}</SelectContent>
            </Select>
          </div>
            ...
            <Label htmlFor="assign-project" className="font-bold">Assign Roll to Project (Optional)</Label>
             <Select value={assignedProjectId} onValueChange={(v) => setAssignedProjectId(v === "none" ? undefined : v)}>
              ...
              <SelectContent>
                ...
                {activeProjects.map((proj) => (<SelectItem key={proj.id} value={proj.id}>{proj.name} ({proj.progress}/{proj.goal})</SelectItem>))}
              </SelectContent>
            </Select>
          </div>
           {isWillForgingActive && (
        ...
        {diceRoll && (
          <div className="pt-6 space-y-4">
            <h3 className="text-lg font-bold text-center font-headline">Roll Results</h3>
            {diceRoll.diceHistories.length > 0 && <DiceDisplay waves={diceRoll.diceHistories} activeCharms={diceRoll.activeCharmIds}/>}
            {diceRoll.totalSuccesses > 0 && (
              <div className="text-center font-bold text-2xl font-headline flex items-center justify-center gap-2">
                {diceRoll.totalSuccesses >= targetNumber ? <CheckCircle2 className="w-8 h-8 text-green-500" /> : <XCircle className="w-8 h-8 text-red-500" />}
                <span>{diceRoll.totalSuccesses} Successes vs TN {targetNumber}</span>
              </div>
            )}
             {diceRoll.automaticSuccesses > 0 && (<p className="text-center text-sm text-muted-foreground">(+{diceRoll.automaticSuccesses} from Charms)</p>)}
            ...
            {activeNarrativeCharms.length > 0 && (
                <div className="pt-4 space-y-2">
                    <Separator />
                    <h4 className="text-center font-headline text-lg text-primary flex items-center justify-center gap-2"><Book className="w-5 h-5"/>Narrative Effects</h4>
        ...
        {aiOutcome && (
          <Card className="mt-6 bg-background/50">
            <CardHeader><CardTitle className="font-headline flex items-center gap-2 text-primary"><Sparkles className="w-6 h-6" />{aiOutcome.outcomeTitle}</CardTitle></CardHeader>
